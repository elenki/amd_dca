# scripts/run_hpo_ae.py (Optuna hyperparam search for NB‐AE)
import optuna
...
def objective(trial):
    ...
    sp = cfg['hyperparam_search']['ae']
    # Suggest model params
    model_cfg = cfg['model_ae'].copy()
    ...
    model_cfg['bottleneck_size'] = trial.suggest_categorical('bottleneck_size', sp['param_distributions']['bottleneck_size'])
...
def main():
    ...
    study.optimize(objective, n_trials=cfg['hyperparam_search']['ae']['n_trials'])
    ...
    # Save best_params
    out = RESULTS / 'ae_best_params.json'
    import json
    with open(out, 'w') as f:
        json.dump(study.best_trial.user_attrs['params'], f, indent=2)
    logging.info(f"Saved best AE params to {out}")
