...
def load_counts(input_type: str, genes: list[str], ids_t: list[str], manifest: dict) -> pd.DataFrame:
    if input_type == "raw":
    ...
    if input_type == "combat":
        ...
        df = pd.DataFrame(full, index=all_ids, columns=genes)
        return df.loc[ids_t].clip(0).round().astype(int)
...
def main(argv=None):
    ...
    # ---- Enhanced Volcano ----
    fc_thresh   = float(dge_cfg.get("fc_threshold", 1.0))
    pval_thresh = float(dge_cfg.get("padj_threshold", 0.05))
    top_n       = int(dge_cfg.get("volcano_top_n", 10))
    ann_file    = RAW / "Human.GRCh38.p13.annot.tsv"
